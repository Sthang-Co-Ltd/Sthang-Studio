...
The Appearance workspace keeps the real video visible and renders the current
project styling using Studio's **native caption preview pipeline**. Instead of
relying on an approximate browser CSS text overlay with divergent web-font
metrics, Studio renders transparent RGBA PNG frames generated by the exact same
local FFmpeg and libass backend used for final MP4 export. Preview and export share
the same native caption layout and rasterization contract before video encoding;
lossy encoding and display scaling may soften pixel edges without changing the
intended typography, line layout, alignment, position, or effects.

Preview frames are mapped precisely to the displayed video rectangle using
`containedVideoFrame` object-fit coordinate projection, properly handling any
letterboxing, pillarboxing, and responsive resizing.

### Native preview architecture

1. **Native backend rendering**: When captions, appearance, or time changes, the
   frontend requests preview frames from `/api/video-export/:projectId/preview`.
   The backend invokes FFmpeg with the native `ass` filter configured for explicit
   libass **complex shaping** (`shaping=complex`).
2. **True alpha difference matte**: To prevent double-opacity artifacts on soft
...
3. **Bounding box detection**: The backend filtergraph runs `bbox` detection on the
   ...
   brackets without altering caption pixels.
4. **Frame caching & bounds**: The frontend maintains an in-memory session cache
   capped at 24 frames and 32 MB of image data. Bounded prefetching fetches up to
   8 future states (`maxPreviewFrames = 8`), while rapid scrubbing aborts obsolete
   pending batches via `AbortController`. Gaps between captions return an empty
   canvas rather than displaying stale frames.
5. **Concurrency & process limits**: Backend preview generation is serialized and
   throttled to a maximum of 2 concurrent FFmpeg subprocesses (`maxConcurrentRenders = 2`),
   preventing daemon CPU starvation during scrubbing.
6. **Fail-closed error handling**: If the local video runtime lacks `subtitlesFilter`
   or if the requested font or bold weight is missing, the preview fails closed
   with an actionable status and a **Retry preview** control. Studio never
   masquerades a broken or missing preview as a CSS approximation.
