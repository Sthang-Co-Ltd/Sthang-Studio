...
5. **Responsive Editor Grid Layout & UI Form Controls** (`apps/web/src/styles.css`, `App.tsx`, `ExportWorkspace.tsx`, `CaptionAppearanceWorkspace.tsx`):
   - Added `minmax(0, 1fr)` and `min-width: 0` to responsive `.editor-grid` to prevent container blowout on mobile.
   - Decoupled form labels from select/input elements using explicit `htmlFor`/`id` pairs.
   - Wired `ConfirmationDialog` for appearance preset deletion.
   - Guarded recovered failed appearance state from clearing the error banner during autosave.

6. **Documentation Updates** (`docs/CAPTIONED-VIDEO-EXPORT.md`, `PRODUCT.md`, `DESIGN.md`):
   ...
   - Recorded layout-locked preview contract and touch target adherence.
...
| `package.json` | `a7d2837920` | Component file for package.json | Application modules / build pipeline | `JSON schema / validator check` | **Changed & Verified**: Updated and verified during audit. |
| `playwright.config.ts` | `3f19a31885` | Component file for playwright.config.ts | Application modules / build pipeline | `npm run typecheck` | **Changed & Verified**: Updated and verified during audit. |
...
| `apps/server/src/services/doctor.ts` | `d1002cd15c` | Component file for doctor.ts | Application modules / build pipeline | `npm run typecheck` | **Retained**: Audited and verified clean. Retained without modification. |
...
| `apps/server/src/services/profile-store.ts` | `497e926ed5` | Manages user profile, preferences, and style presets with file persistence. | apps/server/src/routes/profile.ts, apps/server/src/routes/projects.ts | `npm run typecheck` | **Changed & Verified**: Eliminated redundant normalization helpers by importing authoritative @kcs/shared. Implemented atomic write (temp file + rename) and in-process mutex queue to eliminate read-modify-write race conditions. |
...
| File Path | Blob SHA | Purpose / Scope | Consumers / Callers | Validation Method | Disposition & Findings |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `apps/web/src/App.tsx` | `aad32f5a0e` | Main Studio frontend application component orchestrating video playback, editor workspaces, and modal tools. | apps/web/src/main.tsx | `npm run typecheck` | **Changed & Verified**: Wired onConfirm to confirmation dialog for appearance preset deletion and coupled review-focus select with htmlFor/id. |
...
| `apps/web/src/styles.css` | `946fde1aff` | Global application stylesheet and responsive editor grid layouts. | apps/web/src/main.tsx | `Visual regression / browser tests` | **Changed & Verified**: Fixed editor-grid track sizing on mobile (max-width: 900px) with minmax(0, 1fr) and min-width: 0 on grid children, resolving container overflow during mobile viewport resize. |
...
| `apps/web/src/components/WorkspaceToolsMenu.tsx` | `a6d445cada` | Component file for WorkspaceToolsMenu.tsx | Application modules / build pipeline | `npm run typecheck` | **Changed & Verified**: Updated and verified during audit. |
...
| `apps/web/src/components/job-manager.css` | `907ed2d299` | Styles the Activity / JobManager modal and queue items. | apps/web/src/components/JobManager.tsx | `Visual regression / browser tests` | **Changed & Verified**: Enforced 44px min-height on mobile viewports (<= 620px) for job toolbar buttons with !important override to satisfy accessibility touch target invariant. |
...
| `tests/caption-renderer-native.test.mts` | `e4fb116f2d` | Component file for caption-renderer-native.test.mts | Application modules / build pipeline | `npm run typecheck` | **Retained**: Audited and verified clean. Retained without modification. |
...
| File Path | Blob SHA | Purpose / Scope | Consumers / Callers | Validation Method | Disposition & Findings |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `tests/browser/activity-review.spec.mts` | `2f67a253a4` | Component file for activity-review.spec.mts | Application modules / build pipeline | `npm run typecheck` | **Retained**: Audited and verified clean. Retained without modification. |
