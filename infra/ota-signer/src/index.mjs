...
function isAllowedPayloadPath(path) {
  if (ALLOWED_TOP_LEVEL_FILES.has(path)) return true;
...
function assertSafeArchivePath(rawPath) {
  ...
  if (path.startsWith('/') || /^[A-Za-z]:/.test(path) || path.includes('//') || path.includes('\0')) throw new SignerError('Archive contains an unsafe path.');
...
export async function parseZip(inputBytes, { stripFirstSegment = false } = {}) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (flags & 0x1) throw new SignerError('Encrypted ZIP entries are not supported.');
    ...
    let rawName;
    try { rawName = decoder.decode(bytes.subarray(nameStart, nameEnd)); }
    ...
    const content = method === 0 ? compressed.slice() : await inflateRaw(compressed);
    ...
    if (totalUnpacked > MAX_UNPACKED_BYTES) throw new SignerError('Archive expands beyond the supported size.');
...
async function fetchBounded(url, maximumBytes) {
  ...
  if (length > maximumBytes) throw new SignerError('Accepted Studio source archive is too large.', 413);
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (bytes.byteLength > maximumBytes) throw new SignerError('Accepted Studio source archive is too large.', 413);
  return bytes;
}
...
async function sourceArchive(commit) {
  const bytes = await fetchBounded(`https://api.github.com/repos/${STUDIO_REPOSITORY}/zipball/${commit}`, MAX_ARCHIVE_BYTES);
...
async function stageObject(bucket, commit) {
  ...
  return { key, object, bytes: new Uint8Array(await object.arrayBuffer()) };
}
...
async function handleWebhook(request, env) {
  ...
  const replay = await env.STUDIO_UPDATES.put(replayKey, `${JSON.stringify({ receivedAt: new Date().toISOString(), issueNumber: context.issueNumber, commentId: context.commentId })}\n`, {
    onlyIf: { etagDoesNotMatch: '*' },
